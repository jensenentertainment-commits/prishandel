...
function pickCartItems() {
  ...
  // “Kurv” som ser ekte ut: 3 linjer, alltid utsolgt
  const wanted = [
    "verdighet-premium",
    "mot-limited",
    "fomo-abonnement", // fra RECOMMENDED (hemmelig katalog)
  ];
  ...
  return items as typeof all;
}

export default function CartPage() {
  const items = pickCartItems();

  const subtotal = items.reduce((s, p) => s + p.now, 0);
      ...
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm opacity-70">
            Du har {items.length} varer i kurven. Ingen av dem finnes.
          </p>
        </div>
...
<div className="mt-1 text-[11px] font-semibold opacity-60 truncate">
  SYS: {getLeaks(p.slug, 1)[0]}
        ...
        <aside className="space-y-4">
          ...
          <CheckoutGate total={total} itemsCount={items.length} />
